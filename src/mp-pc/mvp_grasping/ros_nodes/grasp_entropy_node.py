...
class ViewpointEntropyCalculator:
    ...
    def __init__(self):
        ...
        self.grasp_rectangle = rospy.Publisher('grasp_rect', Marker, queue_size=10)
    ...
    def update_service_handler(self, req):
        ...
        with TimeIt('Total'):
            with TimeIt('Update Histogram'):
                ...
                # Do grasp prediction
                depth_crop, depth_nan_mask = process_depth_image(depth, self.img_crop_size, 224, return_mask=True, crop_y_offset=self.img_crop_y_offset)
 #               depth_crop, depth_nan_mask = process_depth_image(depth, self.img_crop_size, 300, return_mask=True, crop_y_offset=self.img_crop_y_offset)
                ...
                angle = (angle + 3 * np.pi / 4) % np.pi  # Wrap [0, pi]
                ...
                width_m = width_img / 224.0 * 2.0 * depth_crop * np.tan(self.cam_fov * self.img_crop_size/depth.shape[0] / 2.0 / 180.0 * np.pi)
            ...
            with TimeIt('Response'):
                ...
                marker.pose.position.x = ret.best_grasp.pose.position.x
                marker.pose.position.y = ret.best_grasp.pose.position.y
                marker.pose.position.z = ret.best_grasp.pose.position.z
                
                marker.pose.orientation.x = ret.best_grasp.pose.orientation.x
                marker.pose.orientation.y = ret.best_grasp.pose.orientation.y
                marker.pose.orientation.z = ret.best_grasp.pose.orientation.z
                marker.pose.orientation.w = ret.best_grasp.pose.orientation.w
                ...
                self.grasp_rectangle.publish(marker)
