...
class BaseGraspController(object):
    ...
    def __execute_best_grasp(self):
            ...
            initial_offset = 0.05
            LINK_EE_OFFSET = 0.078

            # Add some limits, plus a starting offset.
            self.best_grasp.pose.position.z = max(self.best_grasp.pose.position.z, 0.026)  # 0.021 = collision with ground
            self.best_grasp.pose.position.z += initial_offset + LINK_EE_OFFSET  # Offset from end efector position to
            # self.best_grasp.pose.position.y -= 0.02
    ...
    def go(self):
        ...
        while not rospy.is_shutdown():
            ...
            velo_ok = self.velo_control_loop()
            self._in_velo_loop = False
            if not velo_ok:
                rospy.sleep(1.0)
                if self.BAD_UPDATE:
